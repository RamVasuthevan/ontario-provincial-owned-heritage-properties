...
def download_xlsx():
    ...
    )  # Adjust the path to the output folder
...
def extract_data_from_excel(excel_file_path, output_json_path):
    ...
    df["Recognition Type:"] = df["Recognition Type:"].apply(
        lambda x: [x] if x == "Evaluated, but not a Heritage Property" else x.split(",")
    )
    df["Other Name(s):"] = df["Other Name(s):"].apply(
...
if __name__ == "__main__":
    ...
    output_json_path = os.path.join(
        os.path.dirname(__file__), "..", "..", "output", "overview.json"
    )
